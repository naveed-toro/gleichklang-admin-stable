...
public class V01_00_02__ChangeCollation implements SpringJdbcMigration
{
    ...
    private final static String CHAR_SET = "utf8mb4";
     ...
     */
    private void changeDatabaseDefault(JdbcTemplate jdbcTemplate) throws SQLException
    {
        final String databaseName = jdbcTemplate.queryForObject("SELECT DATABASE()", String.class);

        if (databaseName == null || databaseName.trim().isEmpty()) {
            throw new SQLException("No database selected for ALTER DATABASE");
        }

        final String alterDatabase = Joiner.on(" ")
                .join("ALTER DATABASE", databaseName, "CHARACTER SET", CHAR_SET, "COLLATE", COLLATION);

        jdbcTemplate.execute(alterDatabase);
        LOG.info("Changed database '{}' charset to '{}' and collation to '{}'",
                databaseName, CHAR_SET, COLLATION);
    }
    ...
    {
        String convertTable = joiner.join(
                "ALTER TABLE", table, "CONVERT TO CHARACTER SET", CHAR_SET, "COLLATE", COLLATION, ";"
        );
